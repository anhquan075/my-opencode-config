#!/usr/bin/env python3
"""
My OpenCode Config Help Command - All-in-one guide with dynamic command discovery.
...
CATEGORY_GUIDES = {
    ...
    "config": {
        "title": "My OpenCode Config Configuration (.ck.json)",
        "workflow": [
    ...
    "notifications": {
        ...
        "workflow": [
            (
                "1. Set env vars",
                "Add `DISCORD_WEBHOOK_URL` or `TELEGRAM_BOT_TOKEN`+`TELEGRAM_CHAT_ID` to `~/.opencode/.env`",
            ),
            ("2. Add hook", "Add Stop hook to `.opencode/settings.json` (see below)"),
            (
                "3. Test",
                '`echo \'{"hook_event_name":"Stop"}\' | node .opencode/hooks/notifications/notify.cjs`',
            ),
        ],
...
def show_overview(data: dict, prefix: str) -> None:
    ...
    print("# My OpenCode Config Commands")
    print()
    ...
    print(
        f"- New feature: `/{prefix}plan` → `/{prefix}code` → `/{prefix}test` → `/{prefix}git:pr`"
    )
    print(
        f"- Bug fix: `/{prefix}debug` → `/{prefix}fix` → `/{prefix}test` → `/{prefix}git:cm`"
    )
    ...
    print(
        "- `:parallel` variants (e.g., `/code:parallel`) = faster but more tokens, check quota"
    )
...
def show_config_guide() -> None:
    ...
    print("# My OpenCode Config Configuration (.ck.json)")
    print()
    ...
    print()
    print(
        "Guidelines auto-inject on session start. Commands like `/brainstorm` respect them."
    )
    print()
    print("---")
    print()
...
def show_coding_level_guide() -> None:
    ...
    print(
        "2. If 0-5, injects guidelines from `.opencode/output-styles/coding-level-*.md`"
    )
