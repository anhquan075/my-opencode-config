...
const MONOREPO_DIR = '/home/kai/my-opencode-config';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'my-opencode-config-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from my-opencode-config-engineer submodule
  const submodulePath = '/home/kai/my-opencode-config/my-opencode-config-engineer';
...
test('create with partial project match in monorepo', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'cli' should match 'my-opencode-cli'
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'my-opencode-cli', 'Should find my-opencode-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'my-opencode-config' matches multiple projects
  const result = run('create my-opencode-config test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
